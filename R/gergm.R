...
#' @param output_directory The directory where you would like output generated by the GERGM estimation proceedure to be saved. This includes, GOF and parameter estimate plots, as well as a summary of the estimation proceedure and an .Rdata file containing the GERGM object returned by this function.
#' @param output_name The common name stem you would like to assign to all object output by the gergm function. Default value of NULL will not produce any output. Must be a character string or NULL. For example, if "Test" is supplied as the output_name, then 4 files will be output: "Test_GOF.pdf", "Test_Parameter_Estimates.pdf", "Test_GERGM_Object.Rdata", and "Test_Estimation_Log.txt" (not currently implemented)
...
gergm <- function(formula,
  ...
  if(!is.null(output_name)){
    ...
    pdf(file = paste(output_name,"_GOF.pdf",sep = ""), height = 4, width = 6)
    Gof_Plot(GERGM_Object)
    dev.off()

    save(GERGM_Object, file = paste(output_name,"_GERGM_Object.Rdata",sep = ""))

    write.table(GERGM_Object@console_output,file = paste(output_name,"_Estimation_Log.txt",sep = ""),row.names = F,col.names = F,fileEncoding = "utf8")
